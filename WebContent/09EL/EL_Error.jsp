...
<body>
	<fieldset>
		<legend>EL에서 에러가발생하거나 값이 출력되지 않는 경우</legend>
		<h2>+를 숫자가 아닌 문자열 연결에 사용시 (에러)</h2>
		 <ul>
		 	<li>\${"100"+10 }: ${"100"+10 }[정상]</li>
		 	<li>\${"백"+10 } : \${"백"+10 }[에러]</li>
		 	<li>\${"hello"+"el" } : \${"hello"+"el" }[에러]</li>
		 	<li>\${"hello"+"el" } : ${"hello"+="el" }[정상]</li>
		 </ul>
		 
		 
		 <!--
		 EL은 null에 대해 어느정도까지는 예외처리해주므로 아래는 에러가
		 발생하지 않는다.
		   -->
		 ...
		 <!--배열이나 컬렉션처럼 인덱스로 접근하는 경우 .(닷)은 사용불가  -->
		 <ul>
		 ...
		 <!--
		 JSTL의  forEach 태그를 통해 반복출력. 해당문장은 배열이나 
		 컬렉션의 크기만큼 반복하는 확장 for문의 형태로 기술됨
		 -->
		 <h4>배열</h4>
		 ...
		 <ul>
		 	<li>존재하지 않는 속성: \${eldto.addr } :
		 		\${eldto.addr }[PropertyNotFoundException발생]</li>
		 	<!--DTO객체에서 getId()를 추석처리후 테스트해볼것  -->
		...
		<h2>EL변수에 값 할당시 [톰켓8.0부터 정상.이전버전은 에러]</h2>
