...
<body>
<!--
EL영역에서 각 영역에 저장된 속성을 읽을때는 xxxScope계열의 내장객체를 사용한다.
xxxScope.  속성명 혹은 xxxScope['속성명'] 형태로 읽어올수 있고,
속성명이 유일할때는 내장객체를 생략할수 있다. 만약 속성명이 동일하면 가장 
좁은 영역의 속성을 읽어온다.

  -->
	<h2>EL에서 내장객체 사용하기 : xxxScope 계열</h2>
	<h3>각 영역에서 저장된 속성 읽기</h3>
	<ul>
		...
		<li>어플리케이션 ${applicationScope.scopeVar}</li>
	</ul>
	
	<h3>xxxScope 미지정후 속성읽기</h3>
	<ul>
		<li>가장좁은 영역읽음: ${scopeVar }</li>
		<li>위처럼 영역부분을 생략한 후 EL로 나중에 읽게 되면가장 좁은 영역인
			page영역이 읽혀지게 된다.</li>
	</ul>
