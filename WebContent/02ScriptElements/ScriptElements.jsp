...
지시자(Directive) : 페이지속성을 지정하거나 외부문서를 
	include할때 사용한다.
...
<body>
	...
	<%--
	스크립트렛(Scriptlet) : 주로 JSP코드를 작성하고 실행할때
		사용하는 영역이다. 스크립트렛은 head영역, body영역 어디서든
		사용할 수 있다.
	--%>
	<%
	int value1 = 3;
	...
	<%--
	표현식(Expression) : 변수를 웹브라우저에 출력할때 사용한다.
		JS의 document.write()와 동일한 역할을 한다.
		표현식 사용시 주의할점은 문장끝에 ;(세미콜론)을 붙이지 않음.
		사용법 : <%=변수 %>
	 --%>
	...
	<%
		...
		함수에서 JSP의 내장객체를 사용할때는 인자로 전달한후 해당
		타입으로 매개변수를 선언해서 사용해야한다.
		*/
