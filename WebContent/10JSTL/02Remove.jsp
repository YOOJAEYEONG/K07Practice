...
remove태그 : set으로 선언한 변수를 제거할때 사용하는 태그
  ...
  -->
   ...
   <%--  <c:set var="pageVar" scope="request">리퀘스트영역1번</c:set> --%>
    ...
    <c:set var="appVar" value="에플리케이션영역" scope="application"/>
    

<%-- 
xxxScope내장객체를사용하지 않으면 가장 좁은 영역의 변수를 읽어온다. 
	변수명이 서로 다르다면 생략이 가능하다.
--%>
    <h3>삭제전 출력하기</h3>
    <ul>
    	<li>페이지 : ${ pageVar }</li>
    	<li>리퀘스트 : ${requestScope.pageVar }</li>
...
--%>
    ...
    <ul>
    	...
    	<li>리퀘스트 : ${requestScope.pageVar }</li><!--삭제됨  -->
    	<li>리퀘스트2: ${requestVar }</li>
    	<li>세션 : ${sessionVar }</li>
    	<li>어플리케이션 : ${appVar }</li>
    </ul>
