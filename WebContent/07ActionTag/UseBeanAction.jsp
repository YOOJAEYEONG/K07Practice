...
<body>
	...
	<%
	1. request영역에 저장된 객체가있으면 가져온다.
	MemberDTO memberDTO = 
		(MemberDTO)request.getAttribute("member");
		2.만약 객체가 null이면 객체를 생성한다.
		if(memberDTO == null) {
			memberDTO = new MemberDTO();
			3.생성한 객체를 request영역에 저장한다.			
			request.setAttribute("member", memberDTO);
		}
	//액션테그를 사용하면 JSP코드보다 간결하게 표현할 수 있다.
	%>
	--%>
	
	
	<%--
	<jsp:useBean의 id속성과 <jsp:setProperty의 name속성이
	서로 연결된다.
	빈의 set계열()을 통해 값을 설정한다.
	--%>
