...
/* 
	...
	서블릿 클래스가 매핑된다.
	3.사용자의 요청이 서블릿(Controllor)으로 전달되고 서블릿은
	요청을 분석한다.
	4. 분석한 요청에 따라 Model(모델)을 사용하여 기능을 수행한다.
	5. 기능을 수행한후 필요한데이터를 request혹은 session영역에 저장한다.
...
public class SimpleMVC extends HttpServlet{
	...
	처리하기위해 processRequest()메소드로 모든요청을 전달한다.*/
	@Override
	...
	/*doGet() , doPost()의 요청을 그대로 전달받기 위해서는
	아래와 같이 메소드를 기술해야한다. 
	...
	private void processRequest(HttpServletRequest req, HttpServletResponse resp) 
		...
		if(type==null) {
			...
			/*
			서블릿 클래스에서 application 내장객체를 사용하기위해
			...
			if(isMember==true) resultObj = "회원님 방가";
			else	resultObj = "넌 회원이 아님";
